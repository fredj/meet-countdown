...
class CircularTimer extends s {
  constructor() {
    super();
    this.duration = 300;
    this.timeLeft = void 0;
    this.running = false;
    this.paused = false;
    this._timerInterval = void 0;
  }
  addSeconds(seconds) {
  ...
  timer(duration) {
    let remaining = Date.now() + duration * 1e3;
    this._timerInterval = setInterval(() => {
      this.timeLeft = Math.round((remaining - Date.now()) / 1e3);
      if (this.timeLeft === 0) {
        this.reset();
      }
      this.requestUpdate();
    ...
    return $`
        <div class="container">
          <div class="setters">
           ...
           <div class="controls">
              <div class="display-remain-time ${this.paused ? "blink" : ""}">${timeFormatter.format(new Date(timeLeft * 1e3))}</div>
...
__publicField(CircularTimer, "styles", r$2`
    .container {
        position: relative;
        /*
        top: 30px;
  ...
  `);
__publicField(CircularTimer, "properties", {
